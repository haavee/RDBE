...
#include <sys/timeb.h>    //fun debug
#include <sys/types.h>   /* data type definition header file */
#include <stdlib.h>
#include "/usr2/fs/include/params.h"  /* module mnemonics */
...
rack=RDBE; drive=MK5C; //XXXX put that in ctl area
 if( drive==MK5 && (drive_type == MK5B || drive_type == MK5B_BS))
  source=drive;
else if(rack == RDBE)  source=rack;  //XXXX
else if (drive==S2) {
   source=S2;
...
mvwaddstr( maindisp, 2, 8, "fmset - VLBA & Mark IV formatter/S2-DAS/S2-RT/Mark5B time set" );
 ...
 } else if( source == RDBE) {  //XXX
   column=6;  //XXX
   hint_row=12;  //XXX
   form="RDBE   ";  //XXX
  mvwaddstr( maindisp, 4, column, "RDBE        " );  //XXX
 } else if(source == S2) {
...
//  struct timeb tmb;
//  ftime(&tmb);
//  printf("MILLI tmb.millitm  = %d ", tmb.millitm);
//	mvwaddstr( maindisp, 10, 10, ctime(&mytime) );
//=========================================================
