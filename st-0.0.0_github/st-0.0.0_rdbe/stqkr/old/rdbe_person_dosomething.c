...
//rdbe_person=dbe0,pfb    Produces response:
//rdbe_personality/dbe0,type=pfb,fpga=pfbg_1_4_2.bin:!dbe_personality?0
//Direct call of personality gives:
...
//commands used to load perso:
//rdbe_cmd=0,30,dbe_personality=pfbg:pfbg_1_4_2.bin
//rdbe_cmd=0,10,dbe_execute=init;
//rdbe_cmd=dbe0,60,dbe_personality=ddc:ddc_1501383.bin
//rdbe_cmd=0,10,dbe_execute=init;
...
//printf ("DB>%s<>%s<>%s<\n",pers_wanted,pers_loaded,fpga_file);
    ...
    strcat (output,data);
    if(strncmp(pers_loaded,pers_wanted,3) !=0){
        strcat(output,":NOTE:MUST LOAD NEW PERSONALITY ");
        if(strncmp(pers_wanted,"pfb",3) == 0)scmds("load_pfb"); // scmds calls fs procedure
        if(strncmp(pers_wanted,"ddc",3) == 0)scmds("load_ddc");
    }
