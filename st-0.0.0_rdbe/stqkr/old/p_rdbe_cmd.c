...
void p_rdbe_cmd(command,itask,ip)
...
int itask;
{
    ...
    sprintf(rdbe_command, "/usr2/st/arecibo/rdbe_cmd.py -i %s -t %s %s",dbe_ip,timout,command->argv [2]); 
...
//   for(i=0;i<strlen(output);i++){if(output[i]=='\n' || output[i]=='\r')output[i]='\0';}//strip any nl
 ...
 //idummy=strlen(output); printf("DEB5 %d\n",idummy);
