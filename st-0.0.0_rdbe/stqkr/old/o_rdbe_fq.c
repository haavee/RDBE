...
//                       arg0 1   2     3  4  5 6 
{
    ...
    int ibm10,ibm11; //0-15 for PFB or 0-3,0-3 for dbe0/dbe1 in DDC
    int iif; //if number 0 or 1
    int itk; //track number
    int dbe_num; //dbe number
    float tcal1,tcal2;
    char pol; //r or l 
    float rfq,bw; //rf frequency and bandwidth
    float fwant,f1,f2,f1f,f2f,tcal_wanted;
//
    ibm1=itask-60;   //which of 16 channels
    ib=ibm1+1;
...
//make a DDC number
    if(dbe_num == 0)ibm10=ibm1;
    ibm11=ibm1-4;
    if(ibm11 <0)ibm11=0; //just in case
//    printf("DBGFQIBM %d %d %d %d\n",dbe_num,ibm1,ibm10,ibm11);

    if(strncmp(command->argv[1],"if0",3) == 0){ iif=0; } else { iif=1; }
    sscanf(command->argv[2],"%f",&rfq);
